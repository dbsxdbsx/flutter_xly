...
MyFloatPanel 是一个全局浮动面板组件，提供拖拽、贴边、展开/收起等交互功能。通过统一的全局管理器 MyFloatPanel.to，你可以在任意位置控制面板的显示、按钮、状态和样式。
...
- **位置持久化**：写入本应用 `MyStorage` 命名容器，不占用桌面端共享的 `GetStorage.gs`（见 [`.doc/local_storage.md`](local_storage.md)）
...
从当前版本开始，MyFloatPanel 支持“多选禁用”能力（即可同时禁用多个带 id 的按钮），并在示例工程中采用“路由单选策略”：
...
```dart
// 设置禁用联动
MyFloatPanel.to.iconBtn('page1').setEnabled(false);

// 切换禁用联动（如果当前是page1则清空，否则设为page1）
MyFloatPanel.to.iconBtn('page1').toggleEnabled();

// 启用全部（清空禁用联动）
MyFloatPanel.to.iconBtns.enableAll();

```

## 3. API 参考

### 3.1 MyFloatPanel（全局管理器）

#### 静态访问
```dart
MyFloatPanel.to // 获取全局实例
```
...
// 单个图标按钮控制（链式句柄）
MyFloatPanelIconBtnCtrl iconBtn(String id);

// 所有图标按钮集合控制（链式句柄）
MyFloatPanelIconBtnsCtrl get iconBtns;
...
### 3.2 MyFloatPanelIconBtnCtrl（单个按钮句柄）
...
MyFloatPanel.to.disabledIds;
MyFloatPanel.to.disabledIdsRx;      // 监听集合变化

// 判断某个 id 是否被禁用
MyFloatPanel.to.isDisabled('page1');

// 集合级控制：启用全部（清空禁用集合）
MyFloatPanel.to.iconBtns.enableAll();

// 单个按钮链式控制：加入/移除禁用集合
MyFloatPanel.to.iconBtn('page1').setEnabled(false); // 加入禁用集合
MyFloatPanel.to.iconBtn('page1').setEnabled(true);  // 从禁用集合移除
```

bool get isEnabled;
```

### 3.3 MyFloatPanelIconBtnsCtrl（集合句柄）
...
- **跟随窗口缩放**：气泡几何（`gap` / `screenMargin` / `maxWidth` / 默认水平 padding 走 `.w`，圆角与尾巴尺寸走 `.r`）、默认字号（`14.sp`）、默认垂直 padding（`4.h`）均通过 ScreenUtil 缩放，与 MyFloatPanel 主体的尺寸体系协调，避免小窗下游离感。
- **宽度策略（对齐 Material `Tooltip` 默认行为）**：
  ...
  - 调用方可通过 `TooltipTheme(data: TooltipThemeData(constraints: BoxConstraints(maxWidth: ...)))` 覆盖默认上限——这同时控制 MyFloatPanel 气泡和 Flutter 官方 `Tooltip`，无需引入 xly 私有 API。
...
- **手动联动**：任意位置可调用 `MyFloatPanel.to.iconBtn(id).setEnabled(false)` 设置当前禁用联动的 id
- **优先级**：显式设置的 `disabled: true` 优先于联动状态

```dart
// 示例：只有前两个按钮参与联动，第三个按钮独立
MyFloatPanel()..configure(items: [
  MyFloatPanelIconBtn(icon: Icons.home, id: 'home', onTap: () => Get.toNamed('/home')),
  MyFloatPanelIconBtn(icon: Icons.settings, id: 'settings', onTap: () => Get.toNamed('/settings')),
  MyFloatPanelIconBtn(icon: Icons.help, onTap: () => showHelpDialog()), // 无id，不参与联动
]);
```
...
// 使用示例
MyFloatPanel.to.disabledStyle.value = const DisabledStyle.defaultX();
MyFloatPanel.to.disabledStyle.value = const DisabledStyle.dimOnly();
MyFloatPanel.to.disabledStyle.value = DisabledStyle.custom((iconSize) {
...
```dart
// 修改面板样式
MyFloatPanel.to.panelWidth.value = 80.0;
MyFloatPanel.to.backgroundColor.value = Colors.blue;
MyFloatPanel.to.handleFocusColor.value = Colors.cyan; // 顶部handle按钮悬停颜色
MyFloatPanel.to.focusColor.value = Colors.green; // 其他功能按钮悬停颜色

// 修改形状和停靠
MyFloatPanel.to.panelShape.value = PanelShape.rounded;
MyFloatPanel.to.dockType.value = DockType.inside;
```
...
class SpecialPageController extends GetxController {
  ...
  void onReady() {
    super.onReady();
    MyFloatPanel.to.hide(); // 隐藏全局面板
  }

  @override
  void onClose() {
    MyFloatPanel.to.show(); // 离开时恢复显示
    super.onClose();
  }
}
```
...
class BusinessController extends GetxController {
  void onSpecialAction() {
    ...
    if (currentRoute == '/home') {
      MyFloatPanel.to.iconBtn('home').setEnabled(false);
    }
  }
}
```
...
- **全局单例**：MyFloatPanel.to 为全局单例，所有页面共享同一个实例
- **自动联动**：框架会自动处理路由变化的状态联动，无需手动配置

### 6.2 性能建议
- 避免在 build 方法中调用 MyFloatPanel.to.configure
...
3. 在复杂业务场景中，优先使用 `MyFloatPanel.to.iconBtn(id).setEnabled(false)` 进行显式联动
...
**Q: 如何手动控制按钮状态？**
A: 使用 `MyFloatPanel.to.iconBtn('id').setEnabled(false)` 禁用或 `MyFloatPanel.to.iconBtns.enableAll()` 启用全部。

**Q: 禁用样式不生效？**
A: 确保按钮处于禁用状态（当前活跃或显式 disabled: true），并检查 disabledStyle 配置。

**Q: 如何修改悬停时的高亮颜色？**
A: 顶部handle按钮使用 `MyFloatPanel.to.handleFocusColor.value = Colors.yourColor`，其他功能按钮使用 `MyFloatPanel.to.focusColor.value = Colors.yourColor`。

**Q: 如何自定义禁用按钮的外观？**
A: 使用 `MyFloatPanel.to.disabledStyle.value = DisabledStyle.custom(builder)` 设置自定义覆盖Widget。

**Q: 面板在某些页面不显示？**
A: 检查是否调用了 `MyFloatPanel.to.hide()`，使用 `MyFloatPanel.to.show()` 恢复显示。
