...
- **当前版本**：`pubspec.yaml` → `0.45.0`（阶段 D：My* 命名硬切、Selector 拆分、barrel 减 export）。
...
- **最近完成**：0.45.0 — 阶段 D（`MyFloatPanel` / `MySmartDock` 主名、Selector `part` 拆分、`xly.dart` 减 export）。
- **上一版**：0.44.0 — 阶段 C；0.43.0 — 阶段 B；0.42.0 — `MyPaths`。
