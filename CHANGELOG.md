...
- **统一公开 Widget 的尺寸参数契约**：`MyButton`、`MyIcon`、`MyCard`、`MyGroupBox`、`MyTextEditor`、`MySpinBox`、`MyMenuButton`、`MyMenuDivider`、`MyEndOfListWidget` 与焦点边框的显式尺寸现在均按 Flutter 逻辑像素直接使用；响应式值由调用方显式传 `.w/.h/.r/.sp`，组件默认值仍只在内部换算一次。

### Fixed

- **Windows 专注助手误判为无通知权限**：`priorityOnly` / `alarmsOnly` 现在只影响“是否弹横幅”，不再阻止向 Windows Toast API 提交；横幅被静默时通知仍可进入通知中心。插件未初始化、权限拒绝等提前返回路径也统一执行配置允许的应用内 fallback，避免业务把未投递事件误记为已通知。
- **ScreenUtil 二次缩放**：修复显式字号、图标、卡片 margin/elevation、输入框 inset、菜单分隔线等已经适配的值被组件再次换算的问题；修复 `MyTextEditor` 默认圆角、下拉间距和输入框水平 padding 自身重复换算的问题；`MyIcon.tapTargetSize` 可独立设置触控区且不会改变视觉尺寸，也不再受 `hoverShadowRadius` 隐式影响。
...
- **`MyTray.beginExit()` / `isExiting`**：新增对外可调的“进入安全退出态”能力——保留托盘图标、禁用点击与菜单、显示“正在安全退出…”，供自定义清理链在最终 `exit(0)` 前显式驱动；`MyApp.exit()` 默认已自动调用，普通项目无需感知。
- **`exitApp(beforeProcessExit:)`**：`exitApp()` 新增可选回调，桌面端在 `exit(0)` 前于 Flutter Engine 仍可用时执行原生资源（如托盘）的最终销毁。

### Fixed

- **桌面端退出可能停在窗口关闭阶段**：`MyApp.exit()` 释放单实例服务时增加 2 秒上限，辅助资源异常不再阻断最终退出；桌面端在 Flutter Engine 仍可调用插件时完成最终清理后直接 `exit(0)`，不再受 `SystemNavigator.pop` 或窗口关闭保护影响，移动端仍只请求系统导航退出。
- **托盘初始化/销毁竞态与关闭拦截所有权**：`MyTray.destroy()` 会等待在途初始化后再清理，不再因尚未置为 initialized 而提前返回；仅当 close-to-tray 确实由本组件启用 `preventClose` 时才在销毁/禁用时解除，避免残留无处理者拦截或破坏其他窗口 listener。
- **托盘图标与真实进程生命周期不一致**：`MyApp.exit()` 默认让托盘进入“正在安全退出…”状态并禁用交互，图标保持到平台关闭完成、`exit(0)` 前的最后一步再显式销毁；既不会提前暗示进程已结束，也避免 Windows 图标假死到鼠标悬停才消失。
...
- **移除 install → userData 文件迁移**：删除 `MyUserDataFilesMigrator`；`prepare` / `apply` / `userDataDirAndApply` 不再接受 `migrateFileNamesFromInstall`（不做 exe 旁旧文件自动搬迁）。

### Added

- **子入口** `package:xly/picker.dart`：**`MyPicker`**（`dir` / `file` / `files`），薄封装 `file_selector`。
- **`MyUserDataDirSession`**：`prepare` / `apply` 桌面 userData 启动编排（`paths.dart` 导出）。
- **`MyPicker.userDataDirAndApply`**：系统选目录 + 校验 + 可选 warnings → `Session.apply`。
- **`MyUserDataDirBootstrapResult`**：`storedPath`、`storedEvaluation`、`hasInvalidStoredPath`。
- **`Session.apply` / `userDataDirAndApply`**：可选 **`onAfterApply`**（应用侧同步后端数据根、路径缓存等）。
- **`MyPicker.resolveInitialDir`**：未传 `initialDir` 时回退 `MyPaths.userDataDir`。
- **文档**：[`.doc/user_data_picker.md`](.doc/user_data_picker.md)；[`.doc/user_data_paths.md`](.doc/user_data_paths.md) 增补 Mermaid（两层目录、`prepare`、API 速查）。

### Changed

- **设置对话框**：不在库内 export；由应用组合 `MyPicker` + `Validator` + `Session`。

### Docs

- `AGENTS.md` 索引 picker / Session；README 路径示例优先 `prepare`。

## 0.46.0 - 2026-05-20

> **路径模块 Dir 大统一**（breaking）。与 0.47 同日合入主干；CHANGELOG 分版记录便于对照迁移，当前 `pubspec` 版本为 **0.47.0**。

### BREAKING

- **路径辅助类 Dir 化**：`MyUserDataDirectoryStore` → `MyUserDataDirStore`，`MyUserDataDirectoryValidator` → `MyUserDataDirValidator`，`MyUserDataDirectoryValidation` → `MyUserDataDirValidation`（文件重命名同步）。
- **Bootstrap 默认**：`user_data_dir.json` + JSON 字段 `userDataDir`（不再使用 `user_data_directory.json` / `userDataDirectory`）。

### Docs

- [`.doc/user_data_paths.md`](.doc/user_data_paths.md) 同步 Dir 命名；`AGENTS.md` §4.2 路径约定更新。
...
- **`MySelector` ScreenUtil 补全**：修复面板布局中遗漏的硬编码像素值，统一通过 ScreenUtil 进行适配（面板最小宽度 `220.w`、按钮间距 `6.w`、安全边距 `8.w`、面板最小高度 `80.h`、`Positioned` 边界 clamp `8.w`、边框宽 `0.8.w`、阴影模糊半径 `24.r`/`6.r`、阴影偏移 `6.h`/`1.h`）
...
- **修复 FloatBar（浮动面板）展开时窗口缩放导致面板位置错乱**：
