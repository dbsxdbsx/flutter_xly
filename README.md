...
final trayIco = await MyPaths.appDirFile('tray.ico'); // app 轨：exe 旁资源
final config = await MyPaths.userDataDirFile('config.json'); // userData 轨：须已 set 根
...
// final config2 = await MyPaths.userDataDirFile('config.json');
```
