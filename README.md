...
### 包入口（0.47+；路径 Dir 化见 [CHANGELOG](CHANGELOG.md) **0.46**）

| import | 适用场景 |
|--------|----------|
| `package:xly/xly.dart` | 常用全家桶（**含** `MyPaths` / `MyUserDataDir*`；**不含** picker、text_editor / scaffold / selector / smart_dock，见下行） |
| `package:xly/paths.dart` | 仅需路径 API（与 `xly.dart` 中 paths 导出相同；Web 目标请用此入口以选用桩实现） |
| `package:xly/picker.dart` | 系统选目录/文件 `MyPicker.dir` / `file` / `files`（**须**单独 import） |
| `package:xly/app.dart` | 仅需 `MyApp`、路由、窗口 API |
| `package:xly/float_panel.dart` | 仅需 `MyFloatPanel` |
| `package:xly/notify.dart` | 仅需 `MyNotify` |
| `package:xly/tray.dart` | 仅需 `MyTray` |
| `package:xly/text_editor.dart` | 仅需 `MyTextEditor` |
| `package:xly/scaffold.dart` | 仅需 `MyScaffold` |
| `package:xly/selector.dart` | 仅需 `MySelector` |
| `package:xly/smart_dock.dart` | 仅需 `MySmartDock` |

通知与反馈三通道划界见 [`.doc/notify_channels.md`](.doc/notify_channels.md)。
...
26. 全局浮动面板(`MyFloatPanel`)(一个可拖动、可停靠、可展开的浮动面板，支持智能联动、位置持久化记忆)
...
32. 单实例管理(`SingleInstanceManager`)(确保应用只运行一个实例，支持激活已有实例，内置智能僵尸检测避免残留端口阻塞启动)
33. 分段选项卡组件(`MyTabView`/`MySegmentedControl`)(macOS 风格 Tab 切换，支持选中态滑块滑动动画、compact/stretched 两种布局模式、顶部/底部位置、自定义样式和内容切换动画)
34. 拖拽保护区域(`MyDragProtectedArea`)(桌面端专用，解决子组件拖拽手势与窗口拖拽的竞争冲突，支持 `ReorderableListView`、`Draggable` 等拖拽组件与可拖拽窗口共存)
...
36. 路径双轨(`MyPaths` install / userData；`MyUserDataDirStore` / `MyUserDataDirSession` 启动编排) — 见 [`.doc/user_data_paths.md`](.doc/user_data_paths.md)
...
- `path_provider: ^2.1.2` - 路径管理（`MyPaths` / Store bootstrap）
- `file_selector: ^1.0.3` - 系统文件/夹对话框（由 `MyPicker` 封装；应用一般无需直接依赖）
...
- 完整使用指南：[本地](.doc/my_selector_usage.md) | [GitHub](https://github.com/dbsxdbsx/flutter_xly/blob/main/.doc/my_selector_usage.md)
- 在触发 Widget 附近弹出半透明毛玻璃面板，支持搜索、清除项、自定义渲染、键盘导航
- 提供两种模式：`MySelectorController`（Controller 模式，推荐）和 `MySelector.show()`（命令式底层 API）

**Controller 模式（推荐，适合多选择器场景）：**

```dart
// 1. 声明（在 GetxController.onInit 中）
...
// 2. UI 中使用（Builder 提供触发位置的 context）
Builder(builder: (ctx) {
  return Obx(() => MyButton(
...
// 3. 随时从外部操控
colorCtrl.setValue('blue');  // 按 value 设置
colorCtrl.clear();           // 清除
```
...
final result = await MySelector.show<String>(
  ...
  items: [MySelectorItem(value: 'a', title: '选项 A')],
);
final changed = result.changed; // null = 用户点外部关闭，未做选择
if (changed != null) {
  print(changed.item?.title); // null = 清除，非 null = 选中了某项
}
```
...
当端口被占用时，不会盲目假设"已有实例在运行"，而是采用以下智能检测流程：

1. **健康检查**：通过 `GET /health` 端点验证旧实例是否真正存活
2. **旧实例存活** → 确认是真实运行的实例，激活它并退出当前实例
3. **旧实例无响应** → 判定为僵尸进程/残留端口，等待并重试接管（最多 3 次，线性退避）
4. **重试仍失败** → 以警告方式放行，避免阻塞用户

这一机制的主要好处是：开发调试时无需手动配置 `singleInstance: false`，也不会因为上一次调试的僵尸进程/IDE 热重启残留端口而导致窗口无法显示。
...
await MyNotify.to.show("标题", "消息内容"); // 见 .doc/notify_channels.md
...
MyDragProtectedArea(
  child: ReorderableListView(
    children: items,
    onReorder: (oldIndex, newIndex) { /* 排序逻辑 */ },
  ),
)
```
...
- `onPointerDown`：保存当前窗口拖拽状态，并临时禁用窗口拖拽
- `onPointerUp` / `onPointerCancel`：恢复先前保存的窗口拖拽状态

**适用场景（桌面端 + 标题栏隐藏 + 内容区域可拖拽窗口）：**

- `ReorderableListView` 使用 `ReorderableDelayedDragStartListener`（长按拖拽）
- 自定义 `Draggable` 组件
- 任何需要自定义拖拽手势的子区域
...
- 通常只需在真正需要拖拽交互的局部区域包裹，不建议全局套用
- `ReorderableListView` 桌面端默认的拖拽句柄（`ImmediateMultiDragGestureRecognizer`）天然快于 pan，不需要此组件；只有使用 `ReorderableDelayedDragStartListener` 时才会产生竞争

### 使用全局浮动面板

`MyFloatPanel` 是一个全新的全局浮动面板系统，提供拖拽、贴边、展开/收起等交互功能。通过统一的全局管理器 `MyFloatPanel.to`，你可以在任意位置控制面板的显示、按钮、状态和样式。
...
### MyFloatPanel 停靠与展开配置

通过 `configure()` 统一配置面板的停靠行为和展开方向：

```dart
MyFloatPanel()..configure(
  items: [...],
...
```dart
// 设置按钮常亮（高亮显示）
MyFloatPanel.to.iconBtn('your_id').setHighlighted(true);

// 取消按钮常亮
MyFloatPanel.to.iconBtn('your_id').setHighlighted(false);

// 切换按钮常亮状态
MyFloatPanel.to.iconBtn('your_id').toggleHighlighted();

// 查询按钮是否常亮
bool isHighlighted = MyFloatPanel.to.iconBtn('your_id').isHighlighted;

// 清空所有常亮按钮
MyFloatPanel.to.highlightedIds.clear();
```
...
- [路径与 userData](.doc/user_data_paths.md) - install / userData 双轨、`Session.prepare`、何时用 Store / Picker
